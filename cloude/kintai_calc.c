...
int find_worst(const Employee list[], int n);
...
int main(void) {
    ...
    worst_attendance_employee = find_worst(list, EMPLOYEE_COUNT);
    if (worst_attendance_employee < 0) {
        printf("[ERROR] [%s:%d] find_worst() failed\n", __func__, __LINE__);
        goto end;
    }

    worst_late_rate = calc_late_rate(&list[worst_attendance_employee]);
    if (worst_late_rate < 0.0) {
        printf("[ERROR] [%s:%d] calc_late_rate() failed\n", __func__, __LINE__);
        goto end;
    }
    worst_late_rate *= 100; /* 割合をパーセント表示にするため 100 を掛ける */
